...
public:
	VisionToDDS() : Node("vision_to_dds_node") {
	...
	void run() {
		...
		constexpr double timeout = 12.0;
		if (!waitForFirstTransform(timeout)) {
			return;
		}
...
private:
	void navigationParameters() {
		...
		RCLCPP_INFO(this->get_logger(), "Get vehicle_visual_odometry_topic: %s", vehicle_visual_odometry_topic_.c_str());
	...
	void precisionLandParameters() {
		...
		if (enable_precland_) {
			...
			landing_target_pose_publisher_ =
				this->create_publisher<px4_msgs::msg::LandingTargetPose>(landing_target_pose_topic_, 10);
		}
	}
	...
	void publishPrecland(const builtin_interfaces::msg::Time &stamp) {
		...
		try {
			...
			tf2::Vector3 rel_ned;
			rel_ned.setX(
				std::cos(gamma_world_) * rel_world.getX() +
				std::sin(gamma_world_) * rel_world.getY());
			rel_ned.setY(
				-std::sin(gamma_world_) * rel_world.getX() +
				std::cos(gamma_world_) * rel_world.getY());
			rel_ned.setZ(rel_world.getZ());

			tf2::Vector3 abs_ned;
			abs_ned.setX(
				std::cos(gamma_world_) * target_world.getX() +
				std::sin(gamma_world_) * target_world.getY());
			abs_ned.setY(
				-std::sin(gamma_world_) * target_world.getX() +
				std::cos(gamma_world_) * target_world.getY());
			abs_ned.setZ(target_world.getZ());

			px4_msgs::msg::LandingTargetPose msg;
			msg.timestamp = static_cast<uint64_t>(stamp.sec) * 1000000ULL +
	...
	void publishVisionPositionEstimate() {
		...
		try {
			...
			if (last_tf_time_ < transform_stamped.header.stamp) {
				...
				position_body.setX(
					std::cos(gamma_world_) * position_orig.getX() +
					std::sin(gamma_world_) * position_orig.getY());
				position_body.setY(
					-std::sin(gamma_world_) * position_orig.getX() +
					std::cos(gamma_world_) * position_orig.getY());
